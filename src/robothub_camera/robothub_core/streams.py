...
class Streams:
    ...
    def create_video(self, camera_serial: str, unique_key: str, title: str) -> 'StreamHandle':
        """
        ...
        @type unique_key: str
        @param title: Name of the stream in the cloud. Does not have to be unique.
        @type title: str
        """
        ...
        if not isinstance(title, str): raise TypeError('description must be a string')

        if unique_key in self.streams:
            raise ValueError(f'Stream with id {unique_key} already exists')

        wish_id = self._agent_client._create_stream(unique_key, title, camera_serial)
        if self._wait_for_wish_response(wish_id, time_limit=15):
            response = self._wish_responses.pop(wish_id)
            if response['action'] == 'granted':
                assert unique_key == response['body']['unique_key'], 'Stream create response: response id matches but unique_key doesn\'t'
                stream = StreamHandle(self._agent_client, unique_key, camera_serial, title, response['body']['fifo_path'])
                self.streams[unique_key] = stream
                return stream
            elif response['action'] == 'rejected':
                raise Exception(
                    f'Create stream \"{unique_key}\" wish with id \"{wish_id}\": Agent REJECTED wish with code \"{response["reason"]["code"]}\" and message \"{response["reason"]["message"]}\"')
            else:
                raise ValueError(f'Invalid action attribute value in create stream response from agent: \"{response["action"]}\"')
        else:
            raise TimeoutError(f'Create stream \"{unique_key}\" wish with id \"{wish_id}\": No response from Agent')
...
class StreamHandle:
    ...
    def __init__(self, agent_client: 'AgentClient', unique_key: str, camera_serial: str, description: str, fifo_path):
        ...
        self._fifo_pipe = os.open(self._fifo_path, os.O_WRONLY | os.O_APPEND)
        ...
        while not self._stop_event.is_set():
            if not self._write_queue.empty():
                ...
                if len(stream_packet) > 2097152:
                    log.warn(f"Packet of size {len(stream_packet)} of stream with unique key \"{self.unique_key}\" was not sent! Maximum size of packets is limited to 2 MB.")
                else:
                    os.write(self._fifo_pipe, stream_packet)
    ...
    def _destroy(self):
        ...
        os.close(self._fifo_pipe)
        try:
            while self._write_thread.is_alive():
                time.sleep(0.01)
        except BaseException as e:
            log.debug(f'StreamHandle Write thread join excepted with: {e}')
